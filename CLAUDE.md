...
Hold-tap config: balanced flavor, 280ms tapping-term, 175ms quick-tap, 150ms require-prior-idle (formula: 10500/WPM). Shift variants (`hml_s`/`hmr_s`) use the same 150ms require-prior-idle but a shorter 200ms tapping-term so same-hand shift resolves faster (see `decisions/001-home-row-mod-tuning.md`).
...
Brackets on middle/index (stronger fingers). Hyphen, colon, and grave on thumb row.
...
Semicolon is on the SYM2 layer in the same thumb position (hold DEL, tap ;).
